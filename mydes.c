...
static long32 (*sbox_output_perm_table)[64]; /*[%] 8 * 64 * long32:
                                            ...
                                            */
static char (*iperm)[256][8]; /* Initial and final permutations */
static char (*fperm)[256][8]; /*[%] 8 * 256 * 8byte:
                                   8   = 8 Sections, 8 bits per section
                                   256 = 8-bit input
                                   8   = scattered 64-bit output produced 
                                        from 8-bit original input
                              */
...
static void permute(char *inblock, char perm[8][256][8], char *outblock) {
   if(perm == NULL){
      memcpy(outblock, inblock, sizeof(char) * 8);
      return;
   }

   memset(outblock, 0, sizeof(char) * 8);
   for(register int i = 0; i < 8; i++)
      for(register int k = 0; k < 8; k++){
         outblock[k] |= perm[i][(unsigned char)inblock[i]][k];
      }
}
...
static long32 f(ulong32 r, unsigned char subkey[8]){
    ...
    for(register int i = 0, p; i < 8; i++)
        for(register int j = 0; j < 6; j++){
            p = plaintext_32bit_expanded_to_48bit_table[i*6+j]-1;
            if(s[p>>3]&bytebit[p&7])
                plaintext[i] |= bytebit[j+2];
        }
    
    memset(m, 0, sizeof(m));
    for(register int i = 0, row, col; i < 8; i++){
        plaintext[i] ^= subkey[i];
        row = ((plaintext[i]>>5)<<1) | (plaintext[i]&1);
        col = (plaintext[i]>>1) & 15;
        if(i&1) m[i>>1] |= sbox[i][row<<4|col];
        else m[i>>1] |= sbox[i][row<<4|col]<<4;
    }
    
    memset(t, 0, sizeof(t));
    for(register int i = 0, p; i < 32; i++){
        p = sbox_perm_table[i]-1;
        if(m[p>>3]&bytebit[p&7])
            t[i>>3] |= bytebit[i&7];
    }
    
    rval = ((ulong32)t[0]<<24) | ((ulong32)t[1]<<16) | ((ulong32)t[2]<<8) | (ulong32)t[3];
    return rval;
}

/* initialize a perm array */
static void perm_init(char perm[8][256][8], char p[64]){
   memset(perm, 0, sizeof(char) * 8 * 256 * 8);
   for(register int i = 0; i < 8; i++)
      for(register int j = 0; j < 256; j++)
         for(register int k = 0, pos; k < 64; k++){
            pos = p[k] - 1;
            if( ((pos>>3)==i) && (j&bytebit[pos&7]) ){
