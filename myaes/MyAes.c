...
void get_column(unsigned char *p, int j, unsigned char *q);
void put_column(unsigned char *p, unsigned char *q, int j);
...
/*
以下函数的源代码已删除, 这些函数经编译后生成的机器码已转移到aes_lib.lib内:
void build_sbox_inverse(void);
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4]);
void ByteSubInverse(unsigned char *p, int n);
void ShiftRowInverse(unsigned char *p);
...
unsigned char sbox_inverse[256]; // sbox反查表
int key_rounds = 0;
...
void MixColumn(unsigned char *p, unsigned char a[4], int do_mul)
{
   /* (1) 对p指向的4*4矩阵m中的4列做乘法运算;
      ...
      (5) 矩阵m中的4列按以下顺序分别与a做乘法运算:
          ...
          第1列: 由m[0][1], m[1][1], m[2][1], m[3][1]构成
          第2列: 由m[0][2], m[1][2], m[2][2], m[3][2]构成
          第3列: 由m[0][3], m[1][3], m[2][3], m[3][3]构成
...
void aes_encrypt(unsigned char *bufin, unsigned char *bufout, unsigned char *key)
{
   ...
   //printf("[aes_encrypt] check point 0\n");
   for(i=1; i<=key_rounds; i++)
   {  /* 第1至key_rounds轮, 做以下步骤: ByteSub, ShiftRow, MixColumn, AddRoundKey */
      ByteSub((unsigned char *)matrix, 16);
      //printf("[aes_encrypt] check point 1\n");
      MixColumnInverse((unsigned char *)matrix, a, 0); /* 不做乘法, 只做矩阵行转列 */
      //printf("[aes_encrypt] check point 2\n");
...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4]){
   ...
   */
   ...
   memcpy(a, x, 4);
   memcpy(b, y, 4);
   //有一些人的函数里搞幺蛾子非得调用 aes_polynomial_mul(a,b,b)，z改了y跟着改，为此不得不复制一遍
   for(register int k = 0; k < 4; k++){
      z[k] = 0;
      for(register int i = 3-k, j = 0; j < 4; j++){
         z[k] ^= aes_8bit_mul_mod_0x11B(a[i], b[j]);
         i = (i+1)&3;
      }
   }
}
...
void aes_decrypt(unsigned char *bufin, unsigned char *bufout, unsigned char *key){
   ...
   for(register int i = key_rounds; i >= 1; i--){
      AddRoundKey((unsigned char*)matrix, key+i*(4*4));
      if(i != key_rounds)
         MixColumnInverse((unsigned char*)matrix, a, 1);
      else
